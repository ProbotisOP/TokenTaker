...
1. Set `EARLY_FEED_ENABLED=true` to receive new Pump.fun creation events over one PumpPortal WebSocket. Creation subscriptions are free.
...
`OBSERVING → ACCUMULATING → CONFIRMED`, with explicit waiting/rejection reasons:
...
- Default policy: at least 8 seconds observed, 6 buyer addresses, 1 SOL net buying over 10 seconds, ≥65% buy share, ≤35% from one buyer, positive flow in both 5-second halves, and another 3 seconds of persistent confirmation.
